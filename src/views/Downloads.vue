      ...
      try {
        if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
          new Notification('下载完成', {
            body: (t.metadata?.uc?.filename as string) || t.source_url?.split('/').pop() || `任务 ${t.id}`,
            silent: true,
          });
        }
      } catch { /* 通知失败不影响功能 */ }
...
async function confirmCreate() {
  if (source.value === 'url' || source.value === 'magnet') {
    ...
    if (!torrentFile.value) return message.warning('请选择 torrent 文件');
  }
  creating.value = true;
  try {
    if (source.value === 'torrent') {
      // 临时上传 torrent（不入文件树，任务创建后由后端清理）
      const tmp = await api.uploadTmp(torrentFile.value);
      await tasks.create({ source: 'torrent', torrentName: tmp.name });
    } else {
      await tasks.create({ source: source.value, url: url.value.trim() });
    }
...
const statusMeta: Record<string, { label: string; type: 'info' | 'success' | 'warning' | 'error' | 'default'; icon: any }> = {
  queued: { label: '等待中', type: 'default', icon: ClockIcon },
