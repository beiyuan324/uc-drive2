...
> 本项目面向 **AI 编码代理与后续维护者**。README.md 是 git 对外文档（不写内部记忆），**本项目全部项目记忆、决策与坑见 [HANDOVER.md](./HANDOVER.md)**。
...
1. **[HANDOVER.md](./HANDOVER.md)** — 压缩提炼后的项目记忆：架构、关键决策/坑、gopeed/UC 接口、TODO、行动指南。**改代码前必读「三、关键决策 / 坑」。**
...
4. **UC 进度必须用 `g.progress.downloaded`**；**速度用 downloaded 增量**；fsutil VDL 与 `g.progress.speed` 都不可靠（详见 HANDOVER）。
...
详情与验证数据见 [HANDOVER.md 第〇节](./HANDOVER.md)。